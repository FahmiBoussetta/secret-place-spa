...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_PHOTO: 'photo/PARTIAL_UPDATE_PHOTO',
...
export default (state: PhotoState = initialState, action): PhotoState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_PHOTO):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_PHOTO):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_PHOTO):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IPhoto> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_PHOTO,
